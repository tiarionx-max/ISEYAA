...
- ✓ Low-Risk gRPC Extraction — News/Waitlist/Reviews + Scoped Delivery OTP — News, Waitlist, and Reviews each extracted to their own independently-deployable Railway process behind a canary+resilience-wrapped `ClientGrpc` facade, following the `notifications-service` hybrid HTTP+gRPC pattern; Delivery's `VerifyDeliveryOtp` RPC extracted the same way while `RequestDelivery`/`AcceptDelivery`/`CompleteDelivery`/`DeliveryGateway` stay in-process by design (wallet-adjacent/Socket.IO-coupled, GRPC-07x deferred); response-shape gaps in Waitlist's join payload and Reviews' photos/pagination/user-embed were reconstructed via direct Prisma access rather than silently dropped (GRPC-07, GRPC-08) — Phase 21 (8/8 plans, completed 2026-07-21). Verified 10/10 must-haves after a gap-closure round (21-08) fixed a business-exception-to-503 downgrade bug: Reviews' and Waitlist's gRPC controllers weren't wrapping `NotFoundException`/`ForbiddenException`/`BadRequestException`/`ConflictException` in `RpcException`, so every 4xx business rejection silently became a generic 503 once canary was live — fixed by mirroring the already-correct `delivery-otp-grpc.controller.ts` pattern. **Human verification pending:** D-08 sizing-gate SQL re-check and a live end-to-end smoke test of the CR-01/CR-02 fix against a running deployment — see `21-HUMAN-UAT.md`
- ✓ Scheduled Ministry Exports & LGA Heatmap — new `MinistryExportSubscription` model (typed recipients/cadence/status columns, in-place updates) backs a `SUPER_ADMIN`-only CRUD API (Swagger/Postman only, no new admin UI panel by design) and a once-daily `@Cron` digest scheduler guarded by Phase 20's `setNx()` distributed lock; each due subscription's rolling window (`lastSentAt ?? createdAt` → now) is gathered from the 3 existing Phase 14 Ministry reports, rendered as one branded multi-section PDF + combined CSV, and sent via a new `SendgridService.sendMinistryDigest()` wrapped in the already-live `ResilienceService` vendor policy (MIN-08a, MIN-08b, MIN-08c); an LGA×month visitor-density heatmap was added to the Ministry dashboard as a 4th panel, built entirely client-side from data the page already fetches — zero new backend query, zero new mapping dependency (MIN-09) — Phase 22 (4/4 plans, completed 2026-07-21). Verified 8/8 must-haves. Code review (non-blocking) flagged 4 warnings worth a follow-up pass: the scheduler's failure-path status write isn't itself guarded against throwing (could abort the remaining due-subscription loop and double-send), the digest CSV has no formula-injection guard on free-text fields, `setNx()` fails open on Redis errors, and the heatmap's "Unknown"-LGA bucket is folded into the color-scale max but never rendered — see `22-REVIEW.md`

This closes out milestone v2.1 — all 5 phases (18-22) complete.
