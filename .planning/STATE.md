...
last_activity: 2026-07-27 - Completed quick task 260727-d80: Build organiser mobile screens (become-organiser gate, event CRUD, analytics)
progress:
  total_phases: 5
  completed_phases: 5
  total_plans: 27
...
- 2026-07-27, user requested (and a quick task, 260727-dcp, is in flight to address) a mobile auth-entry redesign: remove the non-functional Apple/Google "coming soon" sign-in buttons, make email the default/primary sign-in path (phone becomes secondary, not removed), and close a real gap the user pointed at — email/password registration previously created an account with zero phone verification. Design: `register.tsx` gains a mandatory in-component two-step form→OTP flow (SMS/WhatsApp only, no Email channel) before `POST /auth/register` succeeds, reusing the existing `consumeValidOtp` backend helper.
