...
- [ ] **TRANSPORT-01**: User (with DRIVER role) can create a driver profile, submit vehicle info and license, and await admin KYC approval before going online
- [ ] **TRANSPORT-02**: User can request a ride by specifying pickup/dropoff coordinates, vehicle type (bike/tricycle/car/minibus), and receive a fare estimate before confirming
- [ ] **TRANSPORT-03**: System matches the nearest online, available driver within 60 seconds using Upstash Redis GEORADIUS; user sees driver name, photo, rating, and ETA
- [ ] **TRANSPORT-04**: Driver and rider can see each other's live GPS position updated every 2 seconds via WebSocket for the duration of the trip
- [ ] **TRANSPORT-05**: Fare is calculated with surge pricing applied when demand > 1.5× supply in a given zone; surge multiplier is displayed before confirmation
- [ ] **TRANSPORT-06**: On trip completion, driver earnings (fare × 0.85) are credited to driver wallet immediately; platform fee (15%) is retained
...
- [ ] **DELIVERY-01**: User can request a parcel delivery by providing pickup address, dropoff address, item description, and estimated weight
...
- [ ] **DELIVERY-05**: Rider earnings (delivery fee × 0.80) are credited on delivery confirmation; platform fee (20%) is retained
...
- [ ] **QA-03**: RLS test suite confirms user A cannot read user B's wallet, bookings, orders, or personal data
- [ ] **QA-04**: OWASP ZAP scan on staging returns zero critical findings on wallet, KYC, and auth endpoints
