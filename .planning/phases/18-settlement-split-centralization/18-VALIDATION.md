...
| TBD-05 | TBD | 1 | SETTLE-11c | — | A settled `Transaction` row's stored amount is unaffected by a subsequent `SettlementSplitTier` update (settle once → update tier → re-fetch original `Transaction` → unchanged) | integration | `npm run test -- settlement.service` (or dedicated `settlement-split-immutability.spec.ts`) | ❌ W0 — new dedicated spec recommended | ⬜ pending |
