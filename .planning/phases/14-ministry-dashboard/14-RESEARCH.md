...
2. **(RESOLVED) Does a Stays cancellation path exist post-confirmation, and where?**
   ...
   - **Resolution:** Covered by the same read-time mechanism as Question 1 above (Plan 14-03, implemented) — `getVisitorEntriesByLgaAndMonth()`/`getPurposeBreakdown()` LEFT JOIN to `bookings` by `sourceId` and filter `status NOT IN ('CANCELLED', 'REFUNDED')` at query time (identical mechanism, same SQL block, as Question 1's resolution). This does not depend on locating which specific Stays code path — host cancellation, refund webhook, admin action, or one not yet built — actually sets `status = 'CANCELLED'`/`'REFUNDED'`; whichever mechanism eventually writes that status, the read-time filter picks it up automatically on the next query, with zero additional wiring required. Whether a Stays cancellation path currently exists in the codebase remains genuinely unconfirmed by this research pass, but that fact no longer blocks or changes Plan 14-03's implementation, which is why this question is resolved.
   - Recommendation (superseded by the Resolution above — kept for history): Quick grep for `status: 'CANCELLED'` writes across `stays.service.ts` and any Stays-related webhook/refund handler before finalizing the Open Question 1 mitigation — five-minute check, not a blocking research gap.
</open_questions>
