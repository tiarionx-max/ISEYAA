...
| `backend/apps/delivery-otp-service/src/app.module.ts` | config (DI root) | request-response (stateful: Redis+Postgres) | `backend/apps/notifications-service/src/app.module.ts` | partial — does NOT import `DeliveryModule`, `WalletModule`, `CommonModule`, or `AuthModule`. `DeliveryService` is directly provided as a bare class, along with its full transitive dependency chain (`WalletService`, `S3Service`, `SettlementService`, `PaystackService`, `RefundService`, `ReferenceService`) as bare-class `providers` entries, plus a `{ provide: DeliveryGateway, useValue: <stub> }` token override — because wholesale-importing `WalletModule`/`CommonModule` would mount their `@UseGuards(JwtAuthGuard)`-protected controllers (`WalletController`; `SettlementController`, `UploadController`) with no `JwtStrategy` ever registered in this process (revised 21-06-PLAN.md per checker BLOCKER fix) |
...
```
For `delivery-otp-service`: **revised, post-checker-round-2** — does NOT wholesale-import `DeliveryModule`, `WalletModule`, `CommonModule`, or `AuthModule`. Wholesale-importing `DeliveryModule` would transitively mount `WalletModule`'s `WalletController`/`CommonModule`'s `SettlementController`/`UploadController` (all `@UseGuards(JwtAuthGuard)`) with no `AuthModule`/`JwtStrategy` registered in this process, causing a runtime "Unknown authentication strategy 'jwt'" on any request to those routes — an unintended, broken wallet-mutation surface violating CLAUDE.md's wallet-security constraint. Instead, `app.module.ts` directly provides the 7 bare-class dependency chain `DeliveryService` actually needs (verified via constructor reads): `DeliveryService`, `WalletService`, `S3Service`, `SettlementService`, `PaystackService`, `RefundService`, `ReferenceService`, plus a `{ provide: DeliveryGateway, useValue: <stub> }` override (the real `DeliveryGateway` needs `JwtService`/`AuthModule`, is never touched by `verifyOtp`, and must not be instantiated as a second live Socket.IO server per Roadmap Success Criteria #2). Controllers array is `[DeliveryOtpGrpcController, HealthController]` only — see `21-06-PLAN.md` Task 1 for the full corrected design.
