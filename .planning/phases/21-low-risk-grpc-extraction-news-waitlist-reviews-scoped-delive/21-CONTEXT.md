...
### Review flag resolution scope (added post-research, 2026-07-20)
- **D-07:** `reviews.proto`'s `ResolveReviewFlag` RPC has no `decision`/`resolution` fields (only `review_id`/`resolved_by_id`), but a flag's outcome (RESOLVED vs. DISMISSED) is entirely determined by `decision`. Since the proto is an already-authored, unchangeable contract (Phase 10-03) and amending it is out of scope for this low-risk phase, `resolveFlag` is **not extracted this phase** — it stays in-process alongside `getFlagQueue`/`getFlag` (which already have no RPC coverage at all). All three `ReviewsAdminController` methods keep calling `ReviewsService`/Prisma directly; only `createReview` and `findByTarget`/`ListReviews` go through the new `reviews-service` gRPC extraction.

### Proto/REST shape-reconciliation tradeoffs (added post-research, 2026-07-20)
- **D-08:** Two thin-proto-vs-REST-shape gaps are accepted as implementation approach, not treated as blockers requiring a proto amendment: (1) Waitlist `GetWaitlistStats` — `WaitlistClientService.stats()` fans out one gRPC call per entry in `WAITLIST_SOURCES` (2 parallel calls today) and reassembles the grouped `[{source, count}]` REST shape client-side. (2) Reviews `ListReviews` — `reviews-service` returns ALL reviews for a target in one call (proto has no pagination fields) and the monolith facade paginates the array in-memory to preserve the existing `{data, pagination}` REST envelope. Both preserve REST response *shape* exactly at current data volumes. The planner must add a task to size these against realistic reviews-per-target and waitlist-per-source counts and flag a follow-up (e.g., proto amendment request) if volumes are large enough to risk the P95 < 500ms constraint (CLAUDE.md) — do not silently accept without this sizing check.
