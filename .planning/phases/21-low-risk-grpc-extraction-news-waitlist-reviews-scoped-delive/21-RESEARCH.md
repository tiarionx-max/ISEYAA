...
> All three open questions below were resolved before planning locked task shape: Q1 by CONTEXT.md D-07 (2026-07-20 addendum), Q2 by CONTEXT.md D-08 (2026-07-20 addendum, enforced via a dedicated planner sizing-gate task per service), Q3 by adopting this research's own recommended naming (`news-service`/`waitlist-service`/`reviews-service`), which the planner confirmed and used unmodified.

1. **RESOLVED by D-07.** ~~`ResolveReviewFlag`'s proto request has no `decision`/`resolution` fields, but CONTEXT.md lists it as an RPC this phase implements — how should the extracted service determine RESOLVED vs. DISMISSED?~~
...
2. **RESOLVED by D-08.** ~~Are Pitfalls 1 (Waitlist stats fan-out) and 2 (Reviews in-memory pagination) acceptable engineering tradeoffs, or does "zero client-visible behavior change" require something stricter?~~ D-08 accepts both as the implementation approach, CONDITIONAL on an explicit sizing check — the planner added a blocking checkpoint task to each of 21-03 (Waitlist Task 3) and 21-05 (Reviews Task 4) that must record a PASS verdict against real production/staging volumes before that service's canary flag is ever flipped on.
...
3. **RESOLVED — this research's own recommendation was adopted as-is.** ~~Should `nest-cli.json` project names and Railway service names use `news-service`/`waitlist-service`/`reviews-service` (matching `apps/` directory names) or something else?~~ Plans 21-01 through 21-04 use `news-service`/`waitlist-service`/`reviews-service` throughout (nest-cli.json, Railway service names, Dockerfiles), matching this section's recommendation with no changes.
