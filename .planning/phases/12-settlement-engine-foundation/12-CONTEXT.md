...
- **D-09 (Events):** Net new — add a uniform, platform-wide `events.platform_fee_pct` and `events.govt_levy_pct` in `PlatformConfig` (same KV pattern as Transport/Delivery), applied identically to every event. No per-organizer negotiated rate (no existing field to extend, unlike Marketplace). **Split is 3-way: organizer + Ministry + platform.** `Event.organizerId` is a real `User` with a wallet, and `TourSettlementService` already pays event organizers when a Tour package links to an `Event` (`tour-settlement.service.ts:202-204`) — settling ticket-purchase revenue without an organizer leg would be inconsistent with that existing precedent. Confirmed via user clarification during planning (resolves RESEARCH.md's flagged `[ASSUMED]` open question).
