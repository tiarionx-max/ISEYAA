...
import { Info, Eye, EyeOff } from 'lucide-react-native';
...
export default function ResetPasswordScreen() {
  ...
  const isReady =
    code.length === OTP_LENGTH && newPassword.length >= 8 && newPassword === confirmPassword && !loading;

  async function handleResetPassword() {
    if (!isReady) return;
    setLoading(true);
    try {
      const res = await api.post('/auth/reset-password', { phone, otp: code, newPassword });
      ...
      const msg = getErrorMessage(err, 'Could not reset password. Please try again.');
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.sub}>Enter the 6-digit code sent to {maskedPhone}, then choose a new password.</Text>
