...
async function bootstrap() {
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      package: 'admin',
      protoPath: [
        join(__dirname, '../../../../../packages/proto/admin.proto'),
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription(
      'gRPC-internal microservice — HTTP surface is health-only (/healthz). Real API contract is the gRPC "admin" package in packages/proto/admin.proto.',
    )
    .setVersion('1.0')
    .build();
  ...
  console.log('admin-service gRPC :5006, HTTP healthz :8080');
}
